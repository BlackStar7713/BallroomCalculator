...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.vibbix.ballroom"
...
$(ESPRESSO_TESTS) : app-androidTest.apk app-instrumented.apk
	...
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.vibbix.ballroom))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.vibbix.ballroom; \
	else \
