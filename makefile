...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.vibbix.ballroom "

INSTRUMENTATION_OPTS=" \
--skip-class=com.vibbix.ballroom.R \
\
\
"
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.vibbix.ballroom$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.vibbix.ballroom.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.vibbix.ballroom > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.vibbix.ballroom.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" -eq "0" ]; then \
	...
	$(ADB) shell pm clear com.vibbix.ballroom > /dev/null; \
	fi)
